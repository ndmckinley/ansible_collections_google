...
def main():
    ...
    module = GcpModule(
        argument_spec=dict(state=dict(default='present', choices=['present', 'absent'], type='str'), name=dict(required=True, type='str'), disable_dependent_services=dict(type='bool')))
...
def response_to_hash(module, response):
    return { u'name': response.get(u'name'),u'parent': response.get(u'parent'),u'state': response.get(u'state'),u'disableDependentServices': response.get(u'disableDependentServices'),u'config': ServiceConfig(response.get(u'config', {}), module).from_response() }
...
class ServiceConfig(object):
    ...
    def to_request(self):
        return remove_nones_from_dict({ u'name': self.request.get('name'),u'title': self.request.get('title'),u'apis': ServiceApisArray(self.request.get('apis', []), self.module).to_request() }
)

    def from_response(self):
        return remove_nones_from_dict({ u'name': self.request.get(u'name'),u'title': self.request.get(u'title'),u'apis': ServiceApisArray(self.request.get(u'apis', []), self.module).from_response() }
)
